...
def train_mdnet():
    ...
    for i in range(opts['n_cycles']):
        ...
        for j,k in enumerate(k_list):
            ...
            pos_branch = model(pos_regions,k,out_layer='conv1')
            neg_branch = model(neg_regions,k,out_layer='conv1')

            pos_branch = model(pos_branch,k,training=True)
            neg_branch = model(neg_branch,k,training=True)

            branch_loss = criterion(pos_branch,neg_branch)
